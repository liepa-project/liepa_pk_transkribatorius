...
for(i=ilg-1; i>0; i--)
{
if(strchr("A�E��IY�OU��", eil[i])&&(eil[i]!=0))
   BalPrieb=1;
else
   if((BalPrieb==1)||(eil[i]=='J'))
   {KietMin=(strchr("E��IY�", eil[i+1])&&(eil[i+1]!=0)||(eil[i]=='J')) ? 1:2;  BalPrieb=0;}
Mn[i]=KietMin;
}
...
for(i=ilg-1; i>0; i--)
{
if(strchr("BDGPTKS�Z�C�", eil[i]))
	{
	if((PriebNe==1)&&(eil[i]!=0))
		{SkD=strchr("BDGZ�", eil[i]) ? 1:2; PriebNe=0;}
	}
else
   PriebNe=1;
SkDu[i]=SkD;
